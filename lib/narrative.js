...
import { wrangle } from '../src/auxiliar/auxiliar.js';
import {
  SCENE_WIDTH,
  DEFAULT_HEIGHT,
  GROUP_MARGIN,
  PATH_SPACE,
  LABEL_SIZE,
} from '../src/auxiliar/consts.js';
import FileHandler from '../src/auxiliar/fileHandler.js';
import udpateSourceAnnTab from '../src/auxiliar/updateSourceAnnTab.js';
...
function Narrative() {
  ...
  narrative.changeStoryline = async function (filename) {
    ...
    fileId = fileHandler.currFile();
    updateStorylineAux(originalFile);
    document.getElementById("checkbox-timeline-extended").disabled = false;
  };

  // Change the storyline file according to the user input
  // -----
  //
  narrative.changeStorylineFile = async function (uploadedFile) {
    //originalFile = await fileHandler.file(filename);
    //fileId = fileHandler.currFile();
    fileId = uploadedFile.title;
    updateStorylineAux(uploadedFile);
    udpateSourceAnnTab(uploadedFile.title, "No annotations available")
    document.getElementById("checkbox-timeline-extended").disabled = true;
  };

  // Change the storyline version according to the user selection
  // Normal or extended
  // -----
  //
  narrative.changeStorylineVersion = async function (version, filename) {
    console.log(version, filename)
    let filenameVersion = filename.replace(".json", version + ".json");
    console.log(filenameVersion);
    originalFile = await fileHandler.fileVersion(filenameVersion, filename);
    fileId = fileHandler.currFile();
    updateStorylineAux(originalFile);
    document.getElementById("checkbox-timeline-extended").disabled = false;
  };
  ...
  narrative.layout = function () {
    console.log(scenes)
    computeSceneCharacters(); // Excludes scenes with no characters
    //computeAllSceneCharacters();
    console.log(scenes)
  ...
  //
  // Populate the scenes with characters from the characters array.
  // This method does not exclude characters or scenes by number of appearances
  function computeAllSceneCharacters() {
    let appearances, finished;
    ...
    scenes.forEach(function (scene) {
      ...
      /**
        ...
        // Add this appearance to the map.
        appearances.push({ character: Object(), scene: scene });
    ...
    console.log(scenes, scenes_copy)
    scenes = scenes_copy;
  }
